...
class AbaloneGui(pyglet.window.Window):
    ...
    def reset_game_gui(self, player=0, random_player=False, variant_name='classical', random_pick=False):
        self.game.reset(
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if pos != -1:
            # self.action(pos)  #old version
            a = self.action(pos)
            if a:
                from project.game.UserVsAlphaBeta import DT, random_game
                import time
                time.sleep(1)
                random_game(DT, self)
